...
def return_args():
    ...
    parser.add_argument('--dataset', type=str, default='cose1.0', choices=['esnli', 'cose1.0', 'esnli_ve', 'esnli_nile',
                                                                           "cose_cage"])

    parser.add_argument('--data_dir', type=str, default='../resources/cose_wt5',
                        help='directory path where inconsistent explanations for step 2 and step 4 are located')

    parser.add_argument('--save_dir', type=str, default='../resources/cose_wt5',
...
def match_candidates(args) -> typing.Dict:
    ...
    for i, e in tqdm(enumerate(inconsist_expl), total=len(inconsist_expl)):
        ...
        if args.dataset == 'cose1.0':
            expl_investigate_ = [s.split("explanation")[1].strip() if 'explanation' in s else s for s in expl_set['expl']]
        else:
            expl_investigate_ = expl_set['expl']

        edit_dist = [editdistance.eval(e.lower(), e_s.lower()) for e_s in expl_investigate_]
    ...
    if 'esnli' in args.dataset:
        ...
        original_label = original_data_df['label'].tolist()
        original_expl = [s.split("explanation")[1].strip() if 'explanation' in s else s for s in original_expl]
    else:
    ...
    for idx, i in enumerate(extracted_idx):
        ...
        dist_ = editdistance.eval(inconsist_expl[i].lower(), original_expl[idx_].lower())
        # if inconsist_expl[i] == original_expl[idx_]:
        if dist_ <= 1:
            continue
        else:
            new_dict['pair_id'].append(pairid_)
            ...
            print(original_expl[idx_], inconsist_expl[i])
    return new_dict
